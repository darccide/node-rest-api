  ...
  if (email) {
    ...
    handlers._tokens.verifyToken(token, email, function(tokenIsValid) {
      if (tokenIsValid) {
        // Lookup items
        let menuArray = [];
        _data.list('items', function(err, itemsData) {
          if (!err && itemsData) {
            itemsData.forEach(function(itemName) {
              _data.read('items', itemName, function(err, data) {
                if (!err && data) {
                  menuArray.push(itemName + ': ' + data.price);
                  if (itemsData.length == menuArray.length) {
                    callback(false, menuArray);
                  }
                } else {
                  callback(404);
                }
...
//         if (tokenIsValid) {
//           // Make sure the item doesn't already exist
//           _data.read('items', itemName, function(err, itemData) {
//             if (!err && itemData) {
//               // Update the fields necessary
//               if (itemName) {
...
//               }
//               // Store the new updates
//               _data.update('items', itemName, itemData, function(err) {
//                 if (!err) {
//                   callback(200);
//                 } else {
//                   console.log(err);
...
//               });
//             } else {
//               callback(400, {'Error': 'The specified item does not exist'});
//             }
//           });
//         } else {
